...
public interface QuestionPullRepository extends CrudRepository<QuestionPullEntity, UUID> {
    ...
    @Query("SELECT q FROM QuestionPullEntity q WHERE q.level = :level AND q.uuid NOT IN :excludedIds ORDER BY function('RAND') LIMIT 1")
    Optional<QuestionPullEntity> findRandomByDifficultyExcludingIds(@Param("level") String difficulty, @Param("excludedIds") List<UUID> excludedIds);
